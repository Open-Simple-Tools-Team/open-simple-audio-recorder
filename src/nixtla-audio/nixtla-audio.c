...
//-------------------------------
	...
	//#pragma message("NIXTLA, COMPILING FOR WINDOWS (OpenAL)")
	#include <AL/al.h>
	#include <AL/alc.h>
	#define NIX_OPENAL
#elif defined(__ANDROID__) //Android
	//#pragma message("NIXTLA, COMPILING FOR ANDROID (OpenSL)")
	#include <SLES/OpenSLES.h>
	#include <SLES/OpenSLES_Android.h>
	#define NIX_OPENSL
#elif defined(__QNX__) //QNX 4, QNX Neutrino, or BlackBerry 10 OS
	//#pragma message("NIXTLA, COMPILING FOR QNX / BB10 (OpenAL)")
	#include <AL/al.h>
	#include <AL/alc.h>
	#define NIX_OPENAL
#elif defined(__linux__) || defined(linux) //Linux
	//#pragma message("NIXTLA, COMPILING FOR LINUX (OpenAL)")
	...
	//#pragma message("NIXTLA, COMPILING FOR MacOSX (OpenAL)")
	#include <OpenAL/al.h>
	#include <OpenAL/alc.h>
	#define NIX_OPENAL
#else	//iOS?
	//#pragma message("NIXTLA, COMPILING FOR iOS? (OpenAL)")
	#include <OpenAL/al.h>
	#include <OpenAL/alc.h>
	#define NIX_OPENAL
#endif
...
//++++++++++++++++++++
	...
	#define NIX_PRINTF_ALLWAYS(STR_FMT, ...)		__android_log_print(ANDROID_LOG_INFO, "Nix", STR_FMT, ##__VA_ARGS__)
	...
	#define NIX_PRINTF_WARNING(STR_FMT, ...)		((void)0)
		...
		#define NIX_PRINTF_INFO(STR_FMT, ...)		((void)0)
		#else
		#define NIX_PRINTF_INFO(STR_FMT, ...)		__android_log_print(ANDROID_LOG_INFO, "Nix", STR_FMT, ##__VA_ARGS__)
		#endif
		#define NIX_PRINTF_ERROR(STR_FMT, ...)		__android_log_print(ANDROID_LOG_ERROR, "Nix", "ERROR, " STR_FMT, ##__VA_ARGS__)
		#define NIX_PRINTF_WARNING(STR_FMT, ...)	__android_log_print(ANDROID_LOG_WARN, "Nix", "WARNING, " STR_FMT, ##__VA_ARGS__)
	#elif defined(__QNX__) //BB10
		#ifndef NIX_VERBOSE_MODE
		#define NIX_PRINTF_INFO(STR_FMT, ...)		((void)0)
		#else
			...
			if((*eng->slObject)->GetInterface(eng->slObject, SL_IID_ENGINE, &eng->slEngine)!=SL_RESULT_SUCCESS){
				...
				if((*eng->slEngine)->CreateOutputMix(eng->slEngine, &(eng->slOutputMixObject), 0, NULL, NULL)!=SL_RESULT_SUCCESS){
					...
					if((*eng->slOutputMixObject)->Realize(eng->slOutputMixObject, SL_BOOLEAN_FALSE)!=SL_RESULT_SUCCESS){
						...
						eng->maskCapabilities |= NIX_CAP_AUDIO_CAPTURE;
						return NIX_TRUE;
					}
				}
			}
...
void nixFinalize(STNix_Engine* engAbs){
	...
	if(eng->sourcesArr != NULL){
		...
		for(i=1; i<useCount; i++){ //Source index zero is reserved
			STNix_source* source = &eng->sourcesArr[i];
			if(source->regInUse){
				//Note: reusable Sources are allowed to remain till the end
				if(source->retainCount != 0){
					sourcesInUse++;
				}
...
NixBOOL __nixSourceInitOpenSLPlayer(STNix_EngineObjetcs* eng, const NixUI16 sourceIndex, STNix_source* src, STNix_audioDesc* audioDesc){
	...
	if(slFormatBitPerSample!=0 && slFormatSampleRate!=0){
		...
		//
		const SLuint32 speakers	= (audioDesc->channels == 1) ? SL_SPEAKER_FRONT_CENTER : (SL_SPEAKER_FRONT_LEFT | SL_SPEAKER_FRONT_RIGHT);
