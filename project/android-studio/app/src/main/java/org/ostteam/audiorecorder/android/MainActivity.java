...
public class MainActivity  extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _btnDelete  = (ImageButton)findViewById(R.id.btnDelete);
        {
            ...
            if (parent == null) {
                Log.i("TEST", "_btnMic.getParent() is null");
            } else {
                if (!(parent instanceof View)) {
                    Log.i("TEST", "_btnMic.getParent() is not a View nor RelativeLayout");
                } else {
        ...
        //
        if(!_rec.engineInit()){
            Log.e("OSAudioRecorder", "engineInit failed.");
        }
        //Timer
                ...
                _recHandler.post(new Runnable() {
                    public void run() {
                        ...
                        if(_rec.encoderIsLoaded()){
                            ...
                            if(relProgress == 1.0f){
                                _rec.encoderFinish();
                                Toast.makeText(getBaseContext(), "Encoder done",Toast.LENGTH_SHORT).show();
    ...
    protected void onPause(){
        super.onPause();
        this.btnStop(null);
        Log.i("OSAudioRecorder", "_rec.stop (at onPause).");
    }
    ...
    protected void onStop(){
        this.btnStop(null);
        super.onStop();
        Log.i("OSAudioRecorder", "_rec.stop (at onStop).");
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (grantResults.length != 1 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
            ...
            Toast.makeText(getBaseContext(), "Pemission not granted",Toast.LENGTH_SHORT).show();
        } else {
        ...
        if(exec) {
            if (!_rec.isActive()) {
                ...
                if (!_rec.start(file.getAbsolutePath(), _buffersPerSec, true)) {
                    Log.e("OSAudioRecorder", "_rec.start failed for path '" + file.getAbsolutePath() + "'.");
                    _recError = true;
                    this.syncUserInterface();
                } else {
                    Log.i("OSAudioRecorder", "_rec.start for path '" + file.getAbsolutePath() + "'.");
                    _recError = false;
    ...
    public void btnStop(View view){
        if(_rec.isActive()) {
            if(_rec.stop()) {
                Log.i("OSAudioRecorder", "_rec.stop.");
                this.syncUserInterface();
            }
        } else if(_rec.playerIsLoaded()) {
            if(_rec.playerStop()){
                Log.i("OSAudioRecorder", "_rec.playerStop.");
                this.syncUserInterface();
            }
        }
    }
    ...
    public void btnDelete(View view){
        if(!_rec.isActive()) {
            if(_rec.reset()){
                Log.i("OSAudioRecorder", "_rec.reset.");
    ...
    public void btnPause(View view){
        if(_rec.isActive() && _rec.isRecording()) {
            if(_rec.pause()){
                Log.i("OSAudioRecorder", "_rec.pause.");
                this.syncUserInterface();
            }
        } else if(_rec.playerIsLoaded() && !_rec.playerIsPaused()) {
            if(_rec.playerPause()){
                Log.i("OSAudioRecorder", "_rec.playerPause.");
                this.syncUserInterface();
            }
        }
    }
    ...
    public void btnResume(View view){
        if(_rec.isActive() && !_rec.isRecording()) {
            if(_rec.resume()){
                Log.i("OSAudioRecorder", "_rec.resume.");
                this.syncUserInterface();
            }
        } else if(_rec.playerIsLoaded() && _rec.playerIsPaused()) {
            if(_rec.playerPlay()){
                Log.i("OSAudioRecorder", "_rec.playerPlay.");
                this.syncUserInterface();
            }
        }
    }
    ...
    public void btnShare(View view){
        if(_rec.samplesCount() > 0) {
            String[] formats = { "WAV (sin comprimir)", "FLAC (comprimido)", "OPUS (comprimido, recomendado)" };
                ...
                public void onClick(DialogInterface dialog, int which) {
                    switch (which){
                        case 1:
                            ...
                            if(_rec.encoderIsLoaded()){
                                Log.e("OSAudioRecorder", "other encoder is active");
                                Toast.makeText(getBaseContext(), "Other enconding is in progress.", Toast.LENGTH_SHORT).show();
                            } else {
                                try {
                                    ...
                                    if(!_rec.encoderStart(fileIn.getAbsolutePath(), fileOut.getAbsolutePath(), "flac")){
                                        Log.e("OSAudioRecorder", "encoder failed");
                                        Toast.makeText(getBaseContext(), "Enconder failed to start.", Toast.LENGTH_SHORT).show();
                                    } else {
        ...
        if(!_rec.isActive() && _rec.samplesCount() > 0) {
            ...
            if(!_rec.playerIsLoaded()) {
                ...
                if(!_rec.playerLoad(file.getAbsolutePath(), _buffersPerSec)) {
                    Log.e("OSAudioRecorder", "_rec.playerLoad failed.");
                } else if(!_rec.playerPlay()) {
                    Log.e("OSAudioRecorder", "_rec.playerPlay failed.");
                } else {
                    Log.i("OSAudioRecorder", "_rec.btnPlay.");
                    this.syncUserInterface();
                }
            }
        }
    ...
    private void syncUserInterface(){
        ...
        if(_rec.playerIsLoaded()){
            ...
            if (_recError) {
                ...
                title = "Error, toca para intentar de nuevo";
            } else if (!_rec.isActive()) {
                Drawable btnDraw = _btnMic.getDrawable();
                btnDraw.setTint(this._colorMicGreen);
                if (_rec.samplesCount() <= 0) {
                    title = "Toca para iniciar grabación";
                } else {
                    title = "Toca para volver a grabar";
                ...
                Drawable btnDraw = _btnMic.getDrawable();
                btnDraw.setTint(this._colorMicYellow);
                title = "Pausado, toca para continuar grabación";
                stopVsb = View.VISIBLE;
            } else {
                Drawable btnDraw = _btnMic.getDrawable();
        ...
        //Lower buttons (layout visibles)
        {
            ...
            Log.i("TEST", "visibleCount("+visibleCount+") visibleWidth("+visibleWidth+")");
            //Layout visibles
            if(visibleCount > 0){
                View layout = null;
                //Get parent layout
                ...
                if(layout != null) {
                    ...
                    for (i = 0; i < btns.length; i++) {
                         ...
                         if(btn.getVisibility() == View.VISIBLE) {
                             ...
                             if(params.leftMargin != xLeft){
                                 ...
                                 requestLayout = true;
                                 Log.i("TEST", "["+i+"].setLayoutParams(.leftMargin=" + xLeft + ") (applied)");
                             } else {
                                 Log.i("TEST", "["+i+"].setLayoutParams(.leftMargin=" + xLeft + ") (ignored)");
                             }
                    ...
                    if(requestLayout){
                        ...
                        Log.i("TEST", "requestLayout called");
                    }
                }
            }
        }
